...
public class SWTServerInputDialog extends Dialog {
	...
	public final int INVALID_PORT = -100;
	public final int INVALID_USER = -101;
	public final int INVALID_IP   = -102;
	...
	protected void okPressed() {
		...
		/* Error check the IP and username. These are strings with integer
		 * error codes. The way we will do this is just by appending a null
		 * string to make it a string because Java is awesome like that.
		 * In GUIAPPMAIN we'll have to check it like that, too. I'll make
		 * a note there later probably.
		 */
		if (username.isEmpty()) {
			JOptionPane.showMessageDialog(null, 
					"Error: Username field is empty.", "Error", 
					JOptionPane.ERROR_MESSAGE
			);
